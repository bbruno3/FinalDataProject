...
monthly.tmp %>%
  ...
  labs(title = "Monthly Average for Five Years of Solar Radiation", y = "Average Solar Radiation per Month (C)", x = "Month") +
  geom_line()

# plot all max wind speed in mph within a day
monthly.tmp %>%
  ggplot( aes(x=YM, y=WindSpeedMax_month, color=StationName)) +
  labs(title = "Monthly Average for Five Years of Wind Speed", y = "Average Wind Speed per Month (C)", x = "Month") +
  geom_line()

# plot all relative humidity max within a day
monthly.tmp %>%
  ggplot( aes(x=YM, y=RHMax_month,  color=StationName)) +
  labs(title = "Monthly Average for Five Years of Relative Humidity", y = "Average Relative Humidity per Month (C)", x = "Month") +
  geom_line()

# plot all reference evapotranspiration in inches 
monthly.tmp %>%
  ggplot( aes(x=YM, y=ETo_month, color=StationName)) +
  labs(title = "Monthly Average for Five Years of Evapotranspiration", y = "Average Evapotranspiration per Month (C)", x = "Month") +
  geom_line()

# plot dewpoint 
monthly.tmp %>%
  ggplot( aes(x=YM, y=DewpoinMean_day_month,  color=StationName)) +
  labs(title = "Monthly Average for Five Years of Dew Point", y = "Average Dew Point per Month (C)", x = "Month") +
  geom_line()
